...
output_file = 'bounds_epslatex/bounds_'.n.'_density_vs_time.tex'
...
plot clients_count_file using 1:3 with linespoints ls 1 title "\\texttt{clients\\_count}", \
	dist_x2_file using 1:3 with linespoints ls 2 title "\\texttt{dist\\_x2}", \
	mst_file using 1:3 with linespoints ls 4 title "\\texttt{mst}", \
	vc_file using 1:3 with linespoints ls 5 title "\\texttt{vc}", \
	vc_mst_file using 1:3 with linespoints ls 6 title "\\texttt{vc\\_mst}"
